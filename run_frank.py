...
def run_generate(prompt, use_threshold=False, discount=0., rm_low_prob=False, use_idf=False):
    ...
    with torch.no_grad():
        ...
        outputs = model(input_ids=input_ids, labels=input_ids, output_attentions=True, rm_low_prob=rm_low_prob,
                        use_idf=use_idf, use_entropy=False)
        loss = outputs["loss"][0]
...
def filter_dataset(dataset):
    if dataset["name"] == 'frank':
        not_remain_type = ['OutE']
        filtered_list = [d for d in dataset["dataset"] if d['error_type'] not in not_remain_type]
