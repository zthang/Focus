...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if alibi is None:
            ...
            # )
            attn_mask = torch.ones(query_layer_.size(-2), key_layer_.size(-2), dtype=torch.float16).tril(diagonal=0).to(
                query_layer_.device)
            attn_mask = attn_mask.masked_fill(attn_mask == 0, -float('inf'))
            attn_weight = torch.softmax(
                (query_layer_ @ key_layer_.transpose(-2, -1) / math.sqrt(query_layer_.size(-1))) + attn_mask, dim=-1)
