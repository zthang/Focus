...
import data.utils_misc
from data.utils_misc import download_file_from_google_drive
CNNDM = None
...
class SummaCBenchmark:
            ...
            if "_org" in fn or fn == "test_chen18_reranked.json":
                for aid in dataset:
                    ...
                    clean_dataset.append(
                        {"filename": fn, "label": label, "document": document, "claim": summary, "cnndm_id": aid,
                         "annotations": [label], "dataset": "cogensumm", "origin": "cnndm"})
                ...
                for d in dataset:
                    ...
                    document = self.get_cnndm_document(aid)
                    clean_dataset.append(
                        {"filename": fn, "label": 1, "document": document, "claim": d["correct_sent"], "cnndm_id": aid,
                         "annotations": [1], "dataset": "cogensumm", "origin": "cnndm"})
                    clean_dataset.append(
                        {"filename": fn, "label": 0, "document": document, "claim": d["incorrect_sent"],
                         "cnndm_id": aid, "annotations": [0], "dataset": "cogensumm", "origin": "cnndm"})
    ...
    def load_xsumfaith(self):
        ...
        for k, vs in groups.items():
            ...
            clean_dataset.append(
                {"document": document, "claim": A["summary"], "bbcid": A["bbcid"], "model_name": A["system"],
                 "label": label, "cut": c, "annotations": annotations, "dataset": "xsumfaith", "origin": "xsum"})
    ...
    def load_polytope(self, which_label="overall"):
        ...
        assert which_label in ["overall", "omission", "addition", "duplication",
                               "inaccuracy"], "Unrecognized `which label`"
        ...
        if not os.path.exists(dataset_folder):
            ...
            for model_name in ["BART", "Bert_Ext", "Bert_Ext_Abs", "BottomUp", "PG", "PG_Coverage", "Summa", "TextRank",
                               "seq2seq"]:
                url = "https://github.com/hddbang/PolyTope/raw/master/outputs_with_human_annotation/Human_Annotation_Summarization_%s.xlsm" % (
                    model_name)
                r = requests.get(url)
                with open(os.path.join(dataset_folder, "Human_Annotation_Summarization_%s.xlsm" % (model_name)),
                          "wb") as f:
                    f.write(r.content)
        ...
        for fn in os.listdir(dataset_folder):
            ...
            for i, segment in all_segments.iterrows():
                ...
                if str(segment["ID"]) != "nan":
                    ID2row[segment["ID"]] = {"ID": segment["ID"], "document": segment["Source"],
                                             "claim": segment["Target"], "errors": [], "cut": c}
            ...
            for ID in ID2row:
                ...
                d["inaccuracy_label"] = 0 if "Inaccuracy_internal" in d["errors"] or "Inaccuracy_external" in d[
                    "errors"] else 1
        ...
        if self.cut == "train":
            ...
            with open(os.path.join(dataset_folder, "unpaired_generated_data/data-original/data-train.jsonl"), "r") as f:
                for i, line in enumerate(f):
                    ...
                    datum = {"document": full_text, "claim": D["claim"], "cnndm_id": D["id"], "label": label,
                             "dataset": "factcc", "origin": "cnndm"}
    ...
    def load_summeval(self, key_focus="consistency"):
        ...
        for i, d in enumerate(raw_dataset):
            ...
            clean_dataset.append(
                {"document": document, "claim": d["decoded"], "label": final_label, "model_name": d["model_id"],
                 "cnndm_id": d["id"], "cut": c, "annotations": annotations, "dataset": "summeval", "origin": "cnndm",
    ...
    def load_frank(self):
        ...
        for d in raw_dataset:
            ...
            summary = d["summary"]
            dataset.append({"document": article, "claim": summary, "label": label, "cut": self.cut, "hash": d["hash"],
    ...
    #         dataset_preds = \
    #         scorer.score([d["document"] for d in dataset["dataset"]], [d["claim"] for d in dataset["dataset"]])[
    #             "scores"]
    #
